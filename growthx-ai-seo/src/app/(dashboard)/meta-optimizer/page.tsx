  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#fafafa]">
      ...
      <div className="p-8 max-w-5xl mx-auto space-y-8">
        <div className="bg-white rounded-xl border p-6 border-[#f4f4f5] shadow-sm">
          <div className="flex flex-col gap-4">
            <h3 className="text-sm font-semibold text-[#09090b]">Analyze Webpage</h3>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-[11px] font-medium text-[#71717a] mb-1.5 block uppercase tracking-wider">Target URL</label>
        ...
        {analyzeMut.isPending && (
          <div className="py-20 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-sm font-medium text-[#09090b]">Analyzing content and rewriting tags...</p>
          </div>
        )}
        ...
        {data && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 space-y-6">
              <div className="bg-white rounded-xl border p-5 border-[#e4e4e7] flex flex-col items-center text-center">
                <h4 className="text-[11px] font-semibold text-[#71717a] uppercase tracking-wider mb-4">Overall Health</h4>
                <div className="flex items-end justify-center gap-1 mb-2">
                  ...
                  <span className="text-sm font-medium text-[#a1a1aa] mb-1">/100</span>
                </div>
              </div>

              <div className="bg-white rounded-xl border p-5 border-[#e4e4e7] space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-[13px] font-semibold text-[#09090b]">Current Title</h4>
                    <span className={`text-[11px] font-bold ${getScoreColor(data.titleScore)}`}>{data.titleScore}/100</span>
                  </div>
                  <p className="text-[13px] text-[#3f3f46] p-3 bg-[#f4f4f5] rounded-md border border-[#e4e4e7]">
                    {data.currentTitle || <span className="italic text-[#a1a1aa]">Missing</span>}
                  </p>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-[13px] font-semibold text-[#09090b]">Current Description</h4>
                    <span className={`text-[11px] font-bold ${getScoreColor(data.descriptionScore)}`}>{data.descriptionScore}/100</span>
                  </div>
                  <p className="text-[13px] text-[#3f3f46] p-3 bg-[#f4f4f5] rounded-md border border-[#e4e4e7]">
                    {data.currentDescription || <span className="italic text-[#a1a1aa]">Missing</span>}
                  </p>
                </div>
                <div className="pt-2">
                  <h4 className="text-[12px] font-semibold text-[#09090b] mb-1 flex items-center gap-1.5">
            ...
            <div className="md:col-span-2 space-y-4">
              <h3 className="text-[14px] font-semibold text-[#09090b] flex items-center gap-2">
              ...
              {data.proposedVariations.map((v: any, i: number) => (
                <motion.div 
                  ...
                  key={i}
                >
                  <div className="bg-white rounded-xl border p-0 border-[#e4e4e7] overflow-hidden hover:border-blue-300 transition-colors">
                    {/* Google Search Snippet Preview */}
                    <div className="p-5 bg-white border-b border-[#f4f4f5]">
                    ...
                    {/* Rationale */}
                    <div className="p-3 bg-[#f8fafc] flex items-start gap-2">
                      <CheckCircle2 size={14} className="text-blue-500 mt-0.5 shrink-0" />
                      <p className="text-[12px] text-[#475569]">{v.rationale}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
