...
const GAP_TYPE_LABELS: Record<string, { label: string; tone: string }> = {
  MARKET_GAP: { label: "Market Gap", tone: "var(--color-success-500)" },
  COMPETITOR_WINNING: { label: "Competitor Winning", tone: "var(--color-error-500)" },
  CUSTOMER_MISSING: { label: "You're Missing This", tone: "var(--color-warning-500)" },
  DIFFERENTIATION: { label: "Differentiation Opp.", tone: "var(--color-accent-600)" },
  EMERGING: { label: "Emerging Trend", tone: "var(--color-series-2)" },
  SATURATED: { label: "Saturated", tone: "var(--color-brand-500)" },
};
...
export default function ContentIntelligencePage() {
  ...
  const statCards = [
    ...
    { label: "Content Analyzed", value: stats?.contentAnalyzed ?? 0, icon: Eye, href: "/content-intelligence/competitors", color: "var(--color-series-2)" },
    { label: "Creative Patterns", value: stats?.creativePatterns ?? 0, icon: Layers, href: "/content-intelligence/patterns", color: "var(--color-series-6)" },
    { label: "Open Gaps", value: stats?.contentGaps ?? 0, icon: Target, href: "/content-intelligence/gaps", color: "var(--color-warning-500)" },
    { label: "Campaigns", value: stats?.campaigns ?? 0, icon: Zap, href: "/content-intelligence/campaigns", color: "var(--color-success-500)" },
  ];

  if (!projectId) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <p className="text-sm text-brand-500">Select a project to get started.</p>
      </div>
    );
  }
      ...
      <div className="border-b bg-white px-6 py-5" style={{ borderColor: "var(--color-brand-100)" }}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-accent-600 to-series-6 shadow-sm">
              <Brain size={18} className="text-white" />
            </div>
            <div>
              <h1 className="text-[15px] font-semibold text-brand-950">Content Intelligence</h1>
              <p className="text-[12px] text-brand-500">Understand the market. Discover what works. Create what is next.</p>
            </div>
          </div>
          <Link href="/content-intelligence/strategy">
            <button className="flex items-center gap-1.5 rounded-lg bg-brand-950 px-3 py-1.5 text-[12px] font-medium text-white transition hover:bg-brand-900">
              <Sparkles size={12} />
              Generate Strategy
            </button>
          </Link>
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {statCards.map((card, i) => (
            ...
            >
              <Link href={card.href}>
                <div className="group rounded-xl border bg-white p-4 transition hover:shadow-md" style={{ borderColor: "var(--color-brand-100)" }}>
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ background: `${card.color}18` }}>
                      <card.icon size={15} style={{ color: card.color }} />
                    </div>
                    <ArrowUpRight size={13} className="text-brand-300 transition group-hover:text-brand-950" />
                  </div>
                  <div className="text-[22px] font-bold text-brand-950">{dashboard.isLoading ? "—" : card.value}</div>
                  <div className="mt-0.5 text-[11px] text-brand-500">{card.label}</div>
                </div>
              </Link>
            </motion.div>
          ))}
        ...
        <div className="rounded-xl border bg-white p-5" style={{ borderColor: "var(--color-brand-100)" }}>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-[13px] font-semibold text-brand-950">Intelligence Pipeline</h2>
            ...
            {[
              {
                ...
                action: "Run Classification", mut: classifyMut, color: "var(--color-series-2)",
              },
              {
                step: "2", title: "Detect Patterns", desc: `${stats?.creativePatterns ?? 0} patterns found`,
                action: "Detect Patterns", mut: patternMut, color: "var(--color-series-6)",
              },
              {
                step: "3", title: "Analyze Gaps", desc: `${stats?.contentGaps ?? 0} gaps identified`,
                action: "Analyze Gaps", mut: gapMut, color: "var(--color-warning-500)",
              },
            ].map((s) => (
              <div key={s.step} className="rounded-lg border p-4" style={{ borderColor: "var(--color-brand-100)" }}>
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <span className="text-[12.5px] font-semibold text-brand-950">{s.title}</span>
                </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          {/* Top opportunities */}
          <div className="rounded-xl border bg-white" style={{ borderColor: "var(--color-brand-100)" }}>
            <div className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: "var(--color-brand-100)" }}>
              <h2 className="text-[13px] font-semibold text-brand-950">Top Opportunities</h2>
              <Link href="/content-intelligence/gaps" className="flex items-center gap-1 text-[11px] text-accent-600 hover:underline">
            ...
            <div className="divide-y" style={{ borderColor: "var(--color-brand-100)" }}>
              {dashboard.isLoading ? (
                ...
                dashboard.data.topOpportunities.map((gap) => {
                  const tone = GAP_TYPE_LABELS[gap.gapType]?.tone ?? "var(--color-brand-500)";
                    ...
                    <div key={gap.id} className="flex items-start gap-3 px-5 py-3.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-[12px] font-medium text-brand-950 truncate">{gap.title}</span>
                          <span className="shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-semibold" style={{ color: tone, background: `${tone}18` }}>
                            {label}
                          </span>
                        </div>
                        <p className="mt-0.5 text-[11px] text-brand-500 line-clamp-1">{gap.description}</p>
                      </div>
          ...
          <div className="rounded-xl border bg-white" style={{ borderColor: "var(--color-brand-100)" }}>
            <div className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: "var(--color-brand-100)" }}>
              <h2 className="text-[13px] font-semibold text-brand-950">Creative Pattern Library</h2>
              <Link href="/content-intelligence/patterns" className="flex items-center gap-1 text-[11px] text-accent-600 hover:underline">
                View all <ChevronRight size={12} />
              </Link>
            </div>
            <div className="divide-y" style={{ borderColor: "var(--color-brand-100)" }}>
                ...
                dashboard.data.topPatterns.map((pattern) => (
                  <div key={pattern.id} className="px-5 py-3.5">
                    <div className="flex items-center justify-between">
                      <span className="text-[12px] font-medium text-brand-950">{pattern.name}</span>
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          <div className="text-[9px] text-brand-400">Saturation</div>
                          <div className="text-[11px] font-semibold text-error-500">{pattern.marketSaturation}%</div>
                        </div>
                        <div className="text-right">
                          <div className="text-[9px] text-brand-400">Opportunity</div>
                          <div className="text-[11px] font-semibold text-success-500">{pattern.opportunityScore}</div>
                        </div>
                      </div>
                    </div>
                    <p className="mt-1 text-[11px] text-brand-500 line-clamp-1">{pattern.description}</p>
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {pattern.keyVisualElements.slice(0, 4).map((el) => (
                        <span key={el} className="rounded-full bg-brand-100 px-1.5 py-0.5 text-[9px] font-medium text-brand-600">{el}</span>
                      ))}
                    </div>
                  </div>
                ))
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <motion.div key={link.href} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 + i * 0.06 }}>
              <Link href={link.href}>
                <div className="group flex h-full flex-col rounded-xl border bg-white p-4 transition hover:shadow-md" style={{ borderColor: "var(--color-brand-100)" }}>
                  <link.icon size={16} className="mb-2 text-accent-600" />
                  <div className="text-[12.5px] font-semibold text-brand-950">{link.label}</div>
                  <p className="mt-1 text-[11px] text-brand-500">{link.desc}</p>
                  <div className="mt-auto pt-3 flex items-center gap-1 text-[10.5px] font-medium text-accent-600">
