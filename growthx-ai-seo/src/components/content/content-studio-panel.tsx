...
export function ContentStudioPanel({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="space-y-5">
      {/* Studio Header & Stats */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-brand-100">
        <div>
          <h3 className="text-lg font-bold text-brand-950">AI Content Studio &amp; Auto-Publisher</h3>
          <p className="text-xs text-brand-500 mt-0.5">
            Rank-ready, GEO-optimized articles with 45-word direct answer blocks, comparison matrices, and FAQ schema.
          </p>
        </div>
      ...
      <QueryState
        ...
        emptyBody="Use 'Draft with Content AI' on Competitor Opportunities or click 'Create Custom Article' above to generate rank-ready pieces."
      >
        <Panel
          title="Content pieces"
          subtitle={`${pieces.data?.length ?? 0} total · ${drafted.length} ready to ship`}
          actions={
            drafted.length > 0 && repo.data && (
              <ActionButton
                variant="primary"
              ...
              >
                {runContent.isPending ? "Opening PR…" : selected.length ? `Commit selected (${selected.length})` : "Commit all drafted to repo"}
              </ActionButton>
            )
          }
        >
      ...
      {runContent.data && (
        <div className="rounded-xl border bg-emerald-50 px-4 py-3 text-[12.5px] text-emerald-800 border-emerald-200">
          {runContent.data.status === "AWAITING_REVIEW" ? (
            <div className="flex items-center justify-between gap-2">
              ...
              {runContent.data.pullRequestUrl && (
                <a href={runContent.data.pullRequestUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-bold text-emerald-800 hover:underline">
        ...
        <div className="p-4">
          {repo.data ? (
            <p className="text-[12.5px] text-brand-700">
              ...
              <span className="font-mono text-[11.5px] font-semibold">{repo.data.defaultBranch}</span>. Nothing publishes without review and evidence.
            </p>
          ) : !showConnect ? (
            ...
            <div className="max-w-md space-y-2.5">
              ...
              <div className="flex gap-2 pt-1">
                <ActionButton variant="primary" onClick={handleConnect} disabled={connectRepo.isPending}>
                  {connectRepo.isPending ? "Connecting…" : "Save connection"}
                </ActionButton>
                <ActionButton variant="secondary" onClick={() => setShowConnect(false)}>
                  Cancel
                </ActionButton>
              </div>
            </div>
      ...
      {(runs.data?.length ?? 0) > 0 && (
        <Panel title="Run history &amp; Pull Request Evidence" subtitle={`${runs.data?.length} run(s)`}>
          <Table minWidth={640}>
            <thead>
              <tr>
                <Th>Kind</Th>
                <Th>Status</Th>
                <Th>Files</Th>
                <Th align="right">Pull Request Evidence</Th>
              </tr>
            </thead>
            <tbody>
              {runs.data?.map((run) => (
                  ...
                  <Td align="right">
                    {run.pullRequestUrl && (
                      <a href={run.pullRequestUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11.5px] font-medium text-accent-600 hover:underline">
                        View GitHub PR <ExternalLink size={10} />
                      </a>
                    )}
                  </Td>
                </Tr>
