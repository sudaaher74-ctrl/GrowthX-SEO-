...
export function PlansTab({
  ...
  projectId: string;
  onOpenTab: (tab: "catalog-you" | "catalog-them" | "gaps" | "marketing") => void;
}) {
  const items = useStagedFixItems(projectId || null).filter((i) => i.category === "Business");
  ...
  return (
    <div className="space-y-4">
      ...
      {done.length > 0 && (
        <Panel title={`Done (${done.length})`} subtitle="Run a fresh crawl after changing your website so the other tabs catch up." padded>
          <ul className="space-y-2">
            {done.map((item) => (
              <PlanRow key={item.id} item={item} projectId={projectId} />
            ))}
