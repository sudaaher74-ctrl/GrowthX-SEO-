...
import { CompetitorSetupService, MAX_COMPETITORS } from './competitor-setup.service';
...
describe('CompetitorSetupService', () => {
  ...
  it('stores a competitor with every platform identity given', async () => {
    const created: any = await service.create('p1', {
  ...
  it('accepts a handle however it was pasted', async () => {
    const fromUrl: any = await service.create('p1', {
      websiteUrl: 'acme.com',
      instagramHandle: 'https://www.instagram.com/acmedairy/',
    });
    expect(fromUrl.instagramHandle).toBe('@acmedairy');

    const bare: any = await service.create('p1', { websiteUrl: 'acme2.com', instagramHandle: 'acmedairy' });
  ...
  it('refuses a sixth competitor rather than silently dropping it', async () => {
    ...
    await expect(service.create('p1', { websiteUrl: 'sixth.com' })).rejects.toThrow(BadRequestException);
  });

  it('refuses a competitor already tracked', async () => {
    prisma.competitorDomain.findUnique.mockResolvedValue({ id: 'existing' });

    await expect(service.create('p1', { websiteUrl: 'acme.com' })).rejects.toThrow(/already tracked/);
  });

  it('refuses a website that is not one', async () => {
    await expect(service.create('p1', { websiteUrl: 'not a url' })).rejects.toThrow(BadRequestException);
  });
  ...
  it('will not repoint a competitor at a different company', async () => {
    ...
    await expect(
      service.update('p1', 'c1', { websiteUrl: 'https://globex.com' }),
    ).rejects.toThrow(/cannot be changed/);
  });

  it('edits everything else in place', async () => {
    const updated: any = await service.update('p1', 'c1', { city: 'Pune', mapsName: 'Acme Pune' });
  ...
  it('accepts the unchanged website on an edit', async () => {
    await expect(
      service.update('p1', 'c1', { websiteUrl: 'https://www.acme.com/about', city: 'Pune' }),
  ...
  it('refuses to touch a competitor from another project', async () => {
    ...
    await expect(service.update('p1', 'other', { city: 'Pune' })).rejects.toThrow(NotFoundException);
