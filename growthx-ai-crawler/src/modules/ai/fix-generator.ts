...
export interface FixPlan {
  ...
  heuristic: () => { proposedValue: string; codeSnippet: string };
}

const TEXT_SCHEMA = (field: string, guidance: string) => ({
  type: 'object',
...
export function humanizeSlug(url: string): string {
  try {
...
function describe(page: PageContext): string {
  const lines = [`URL: ${page.url}`];
  if (page.title) lines.push(`Current title: ${page.title}`);
  if (page.metaDescription) lines.push(`Current meta description: ${page.metaDescription}`);
  if (page.h1?.length) lines.push(`H1: ${page.h1.join(' | ')}`);
  if (page.h2?.length) lines.push(`H2s: ${page.h2.slice(0, 8).join(' | ')}`);
  if (page.wordCount) lines.push(`Word count: ${page.wordCount}`);
  return lines.join('\n');
}
...
function breadcrumbFor(url: string) {
  let segments: string[] = [];
  let origin = '';
  try {
    const parsed = new URL(url);
...
export function renderFromModel(
  plan: FixPlan,
  parsed: Record<string, any>,
  page: PageContext,
): { proposedValue: string; codeSnippet: string } | null {
  ...
  switch (plan.fixType) {
    ...
    case 'BREADCRUMB_SCHEMA': {
      ...
      try {
        ...
        if (!jsonLd || typeof jsonLd !== 'object' || !('@type' in jsonLd)) return null;
        return { proposedValue: `${(jsonLd as any)['@type']} JSON-LD`, codeSnippet: jsonLdScript(jsonLd) };
      } catch {
        return null;
      }
    }
