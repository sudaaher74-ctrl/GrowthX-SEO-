...
export function buildVisibilityReport(
  ...
  const inWindow = (check: ReportableCheck, start: Date, end: Date) =>
    check.checkedAt >= start && check.checkedAt < end;
  ...
  const positions = cited.map((c) => c.position).filter((p): p is number => typeof p === 'number');
  ...
  // ---- per assistant
  const assistantMap = new Map<AiAssistant, { checked: number; cited: number }>();
  ...
  const byAssistant: AssistantBreakdown[] = [...assistantMap.entries()]
  ...
  const weekMap = new Map<string, { checked: number; cited: number }>();
  for (const check of ran) {
  ...
  const trend: TrendPoint[] = [...weekMap.entries()]
