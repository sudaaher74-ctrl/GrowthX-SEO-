 ...
 * the crawl runs.
 *
 * `markUrlVisited` now returns `{ alreadyVisited, limitReached }` instead of
 * a plain boolean so the caller can distinguish duplicates from ceiling hits,
 * and update per-job stats accordingly.
 */
describe('CrawlerService — page ceiling', () => {
  ...
  /** True when the URL should not be fetched (either duplicate or limit hit). */
  const shouldSkip = (r: { alreadyVisited: boolean; limitReached: boolean }) =>
    r.alreadyVisited || r.limitReached;
  ...
  describe('with Redis', () => {
    ...
    it('returns limitReached when the ceiling is hit via Redis', async () => {
      const client = redis();
      const service = build(client);
