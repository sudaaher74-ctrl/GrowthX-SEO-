...
exports.STRATEGY_SYSTEM_PROMPT = exports.STRATEGY_SCHEMA = void 0;
exports.buildStrategyPrompt = buildStrategyPrompt;
/** The plan we ask the model to produce. */
exports.STRATEGY_SCHEMA = {
    type: 'object',
    properties: {
        businessSummary: {
            type: 'string',
...
exports.STRATEGY_SYSTEM_PROMPT = 'You are a growth strategist producing a plan for one specific business. ' +
...
function buildStrategyPrompt(evidence) {
    ...
    const sections = [
        ...
        `# Actual pages on the site\n${bullet(site.samplePages.map((p) => `${p.url} — "${p.title ?? 'untitled'}" (${p.wordCount} words)`))}`,
        `# AI assistant visibility\n` +
            (aiVisibility.trackedPromptCount === 0
                ...
                : `Tracked prompts: ${aiVisibility.trackedPromptCount}\n` +
                    ...
                    `Per assistant:\n${bullet(aiVisibility.byAssistant.map((a) => `${a.assistant}: ${a.citationSharePct}%`))}\n` +
                    `Prompts where competitors are cited and this business is not:\n${bullet(aiVisibility.lostPrompts.map((p) => `"${p.prompt}" — cited instead: ${p.competitors.join(', ')}`))}`),
...
//# sourceMappingURL=strategy-evidence.js.map